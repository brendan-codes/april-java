...
class DLList {
    ...
    exists(val) {
        runner = this.head;     //start with the head make it our runner
        while(runner){
            if(runner.data===val){  //if the data in the node = val
            return true;            //return true
            }
            runner = runner.next;   //if they dont equal each othe move the runner up to the next node
        }
        return false;               //if nothing found/ or empty, return false
    }
