...
class slStack {
    ...
    copy() {
		const tS = new slStack();
		while (!this.isEmpty()) tS.push(this.pop());
		const nS = new slStack();
		while (!tS.isEmpty()) nS.push(new Node(this.push(tS.pop()).peek().data));
		return nS;
	}
    ...
    reverse() {
		const tQ = new Queue();
		while (!this.isEmpty()) tQ.enqueue(this.pop());
		while (!tQ.isEmpty()) this.push(tQ.dequeue());
	}
