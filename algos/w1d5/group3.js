...
class SLL {
    ...
    reverse() {
        let prev = null; // store previous node
        let nextTemp = null; // store the next node before changing reference
	    let runner = this.head; // current node

        while (runner != null){
            nextTemp = runner.next; // save next in the temp var
            runner.next = prev; // reverse here
            prev = runner; // prev node becomes current node
            runner = nextTemp; // current becomes the next node from earlier?
        }
        this.head = prev; // prev node will be the head node?
    }
